      ...
      <nav className="sticky top-0 z-50 border-b border-[#27272a] bg-[#09090b]/85 backdrop-blur-xl" role="navigation" aria-label="Main navigation">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={openSearch} aria-label="Search repositories (Command+K)"
              ...
              <kbd className="hidden sm:inline text-[10px] text-zinc-500 bg-[#27272a] px-1.5 py-0.5 rounded font-mono">⌘K</kbd>
            </button>
            
            {session ? (
              <button onClick={() => signOut()} aria-label="Sign out"
                className="hidden md:flex items-center px-3 py-2 text-sm text-zinc-400 hover:text-zinc-200 rounded-[14px] hover:bg-[#18181b]/50 transition-colors duration-200">
