...
const icons: any = {
  repos: GitFork, issues: AlertCircle, recommendations: Sparkles, portfolio: BookOpen,
  search: Search, activity: ActivityIcon, discover: Compass, error: AlertCircle,
}
...
const EmptyState = (props: any) => {
  const { kind = "repos", title, description, action, secondaryAction } = props
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {action ? (
          action.href ? (
            <Link href={action.href} className="h-10 px-5 bg-white text-zinc-900 rounded-[14px] text-sm font-semibold inline-flex items-center gap-2 hover:bg-zinc-200 transition-colors">{action.label} <ArrowRight className="w-4 h-4" /></Link>
          ) : action.onClick ? (
            <button onClick={action.onClick} className="h-10 px-5 bg-white text-zinc-900 rounded-[14px] text-sm font-semibold inline-flex items-center gap-2 hover:bg-zinc-200 transition-colors">{action.label} <ArrowRight className="w-4 h-4" /></button>
          ) : null
        ) : (
          <Link href={d.href} className="h-10 px-5 bg-white text-zinc-900 rounded-[14px] text-sm font-semibold inline-flex items-center gap-2 hover:bg-zinc-200 transition-colors">{d.label} <ArrowRight className="w-4 h-4" /></Link>
        )}
        {secondaryAction && (
          secondaryAction.href ? (
            <Link href={secondaryAction.href} className="h-10 px-5 bg-[#18181b] text-white border border-[#27272a] rounded-[14px] text-sm font-medium inline-flex items-center gap-2 hover:bg-[#27272a] transition-colors">{secondaryAction.label}</Link>
          ) : secondaryAction.onClick ? (
            <button onClick={secondaryAction.onClick} className="h-10 px-5 bg-[#18181b] text-white border border-[#27272a] rounded-[14px] text-sm font-medium inline-flex items-center gap-2 hover:bg-[#27272a] transition-colors">{secondaryAction.label}</button>
          ) : null
        )}
      </div>
    </div>
