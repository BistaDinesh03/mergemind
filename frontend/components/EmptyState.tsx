...
export function EmptyState({ type = "repos", title, description, action, secondaryAction }: EmptyStateProps) {
    ...
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {action ? (
          ...
          <Link href={config.defaultAction.href} className="h-10 px-5 bg-white text-zinc-900 rounded-[14px] text-sm font-semibold inline-flex items-center gap-2 hover:bg-zinc-200 transition-colors">
            {config.defaultAction.label} <ArrowRight className="w-4 h-4" />
          </Link>
        )}
