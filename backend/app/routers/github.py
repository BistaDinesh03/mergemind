...
async def _detect_repo_setup(owner: str, repo: str, language: str, topics: list) -> dict:
    ...
    try:
        root_contents = await github_client.request(f"https://api.github.com/repos/{owner}/{repo}/contents/")
    ...
    if "requirements.txt" in file_names:
        ...
        setup["setup_commands"] = ["python -m venv venv", "source venv/bin/activate", "pip install -r requirements.txt"]
...
def _suggest_files(language: str, title: str, body: str, repo_setup: dict = None) -> list:
    ...
    if "bug" in text or "fix" in text:
        suggestions.append(f"Source files in {lang or 'src'}/ directory")
    if "test" in text:
        suggestions.append("Test files")
...
async def contribution_guide(
    ...
    if not issue_data:
        return {
            ...
            "issue": {
                "number": issue_number,
                "title": "Issue not found or closed",
                "url": f"https://github.com/{owner}/{repo}/issues/{issue_number}",
                "labels": [],
                "is_beginner_friendly": False
            },
            "ai_summary": "This issue may have been closed, moved, or the repository is no longer available.",
            "scoring": {"overall": 0, "verdict": "Unavailable"},
            "related_prs": [],
            "repo_setup": {},
            "guide": {
    ...
    # Fetch repo data with fallback
    repo_data = None
    try:
        repo_data = await github_client.request(f"https://api.github.com/repos/{owner}/{repo}")
    except HTTPException:
        repo_data = None
    
    if not repo_data:
        repo_data = {"language": "", "topics": [], "default_branch": "main", "stargazers_count": 0}
    
    # Fetch related PRs with fallback
    related_prs_data = None
    try:
        related_prs_data = await github_client.request(
            "https://api.github.com/search/issues",
            {"q": f"repo:{owner}/{repo} type:pr state:closed is:merged", "sort": "updated", "order": "desc", "per_page": 3}
        )
    except HTTPException:
        related_prs_data = None
    
    # Detect repo setup with fallback
    repo_setup = {}
    try:
        repo_setup = await _detect_repo_setup(owner, repo, repo_data.get("language", ""), repo_data.get("topics", []))
    except Exception:
        repo_setup = {}
    ...
    return {
        ...
        "scoring": IssueScorer.calculate(issue_data, repo_data),
        ...
        "guide": {
            ...
            "steps": [
                ...
                {"step": 3, "title": "Install Dependencies", "description": f"Setup with {repo_setup.get('package_manager', 'manual')}", "commands": repo_setup.get("setup_commands", [])},
                ...
                {"step": 5, "title": "Make Changes", "description": "Implement the fix described in the issue", "files_to_edit": _suggest_files(repo_data.get("language", ""), issue_data.get("title", ""), issue_data.get("body", "") or "", repo_setup)},
